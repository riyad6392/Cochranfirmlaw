      ...
      <div class="col-12">
        <div class="card mb-4">
          <div class="card-header pb-0 d-flex" style="justify-content: space-between;">
            <h6>General Links</h6>
            <a class="btn btn-sm btn-primary" href="{{route('add.general.link.view')}}">Add New</a>
          </div>
          <div class="card-body px-0 pt-0 pb-2">
            ...
            <div class="table-responsive p-0">
              <table class="table align-items-center mb-0">
                ...
                <tbody>
                    @foreach ($generalLinks as $generalLink)
                    <tr>
                        <td>
                          <div class="d-flex px-2 py-1">
                            <div>
                              <img src="{{ asset($generalLink->content_image) }}" class="avatar avatar-sm me-3" alt="user1">
                            </div>
                            <div class="d-flex flex-column justify-content-center">
                              <h6 class="mb-0 text-sm">{{strlen($generalLink->title) > 35 ? substr($generalLink->title, 0, 35) . '...' : $generalLink->title}}</h6>
                              <p class="text-xs text-secondary mb-0">{{$generalLink->slug}}</p>
                            </div>
                          </div>
                        </td>
                        <td>
                            <img src="{{ asset($generalLink->banner_image) }}" class="avatar avatar-sm me-3" alt="user1">
                        ...
                        <td class="align-middle text-center text-sm">
                            <p class="text-xs text-secondary mb-0">{{strlen($generalLink->description) > 50 ? substr($generalLink->description, 0, 50) . '...' : $generalLink->description}}</p>
                        </td>
                        <td class="align-middle">
                          <a href="{{route('edit.general.link.view',$generalLink->id)}}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                          ...
                          <a href="{{route('delete.general.link',$generalLink->id)}}" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
